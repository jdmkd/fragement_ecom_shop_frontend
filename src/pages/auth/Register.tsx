...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = data.username ? data.username[0] : data.email ? data.email[0] : data.detail || "Registration failed.";
        showToast({
          type: "error",
          title: "Registration Failed",
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 w-full max-w-md mx-auto">
      ...
      <p className="text-gray-600 dark:text-gray-400 mb-8 text-center">Create your account to get started</p>
      <form onSubmit={handleSubmit} className="space-y-6 w-full">
        <div className="space-y-2">
          <Label htmlFor="username" className="text-base">Username</Label>
          <Input id="username" name="username" type="text" placeholder="johndoe" value={username} onChange={handleChange} required className="p-3 text-base" />
        </div>
        <div className="space-y-2">
          <Label htmlFor="fullname" className="text-base">Full Name</Label>
          <Input id="fullname" name="fullname" type="text" placeholder="John Doe" value={fullname} onChange={handleChange} required className="p-3 text-base" />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Input id="phonenumber" name="phonenumber" type="tel" placeholder="+1234567890" value={phonenumber} onChange={handleChange} required className="p-3 text-base" />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Input id="password2" name="password2" type="password" placeholder="********" value={password2} onChange={handleChange} required className="p-3 text-base" />
        </div>
